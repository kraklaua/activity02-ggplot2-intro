...
**Response**: The gentoo species has a larger flipper length than the
other two species.
...
**Response**: The boxplot can show the minimum, maximum, and the
interquartiles. The jitterplot shows all individual points.
...
**Response**: The md file is easier to read and looks more put together
where the rmd shows the code without the graphs.
...
**Response**: I prefer the jitterplot laid over the boxplot so you can
see all the points. When the boxplot is laid over the jitterplot then
the boxplot covers some of the points.
...
``` r
  ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species)) +
  geom_boxplot() + geom_jitter(aes(color=species))
```
...
**Response**: Mapping in the ggplot or the geom\_boxplot will fill the
boxplot.
